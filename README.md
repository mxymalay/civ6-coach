...
- 固定后自动切换透明游戏叠加层：白字深色阴影、空心「快速建议」，无卡片和实色背景；生成时按钮变为「停止生成」。移入顶部显示暂停、取消固定、收起；拖动顶部移动、拖动边缘缩放，最小 300×200。取消固定恢复完整主题界面。
- 叠加层使用右下角小号空心按钮。旁边的双向箭头是始终可见的 30×30 缩放手柄；普通小窗也提供同一手柄，无需精准抓住窗口边缘。
- 应用内快捷键：⌘⇧J 显示/收起小窗，⌘⇧O 固定/取消固定；不是游戏内全局热键。空心按钮按当前状态显示开启陪练、配置 AI、快速建议或停止生成；开启或固定本身不发送 AI 请求。
